...
subsampling <- function(data, delta=1, theta=0.9, gamma=5, epsilon = 0, important=c(0:5000))
{
...
#", Area ",  paste(unique(data$Area), collapse="/"), ", Quarter ", paste(unique(data$Quarter), collapse="/"),  ", Year", paste(unique(data$Year),collapse="/" ),"\n"), gp=gpar(fontsize=27, font=2)));
...
#PF.ks <- ecdf(subset(data, LengthClass_cm %in% important)$LengthClass_cm);

#print(ks.test(subset(subsample, LengthClass_cm %in% important)$LengthClass_cm, subset(contrsubsample, LengthClass_cm %in% important)$LengthClass_cm));

#print(ks.test(subset(subsample, LengthClass_cm %in% important)$LengthClass_cm, PF.ks));

#print(ks.test(subsample)$LengthClass_cm, PF.ks));


subsample_full <- rbind(data_not_important, subsample);

contrsubsample <- subset(contrsubsample, !(rownames(contrsubsample) %in% rownames(to_csub)));

#print(dim(contrsubsample))
#print(nrow(subsample_full))


#print(intersect(rownames(to_csub),rownames(contrsubsample)))
