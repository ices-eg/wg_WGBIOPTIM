...
if (is.null(ind_modes_k)) ind_modes_k <- length(xk$freq);
...
#if (length(modes_robust)==0) modes_robust <- as.numeric(as.character(x$LengthClass_cm[nrow(x)]))
...
if (length(modes_robust)==0) modes_robust <- c(as.numeric(as.character(subset(x, freq==max(subset(x, LengthClass_cm %in% modes_signif)$freq))$LengthClass_cm)));

if (length(antimodes_robust)==0) antimodes_robust <- c(as.numeric(as.character(subset(x, freq==min(subset(x, LengthClass_cm %in% antimodes_signif)$freq))$LengthClass_cm)));

if (length(modes_robust)==1 & length(antimodes_robust)>1) antimodes_robust <- c(0);

### CORRECTION ###

frame_correction_modes <- data.frame(modes_robust, rep("m",length(modes_robust)));
names(frame_correction_modes) <- c("value","type");
frame_correction_antimodes <- data.frame(antimodes_robust,rep("a",length(antimodes_robust)));
names(frame_correction_antimodes) <- c("value","type");

frame_correction <- rbind(frame_correction_modes,frame_correction_antimodes);

frame_correction <- frame_correction[order(frame_correction$value),];

if (frame_correction[1,]$type=="a" & frame_correction[1,]$value!=0) frame_correction <- frame_correction[-1,];

if (frame_correction[nrow(frame_correction),]$type=="a") frame_correction <- frame_correction[-nrow(frame_correction),];

if (nrow(frame_correction)>1)

{

value_exclude <- c();

for (i in 1:(nrow(frame_correction)-1))
{
if (frame_correction$type[i]==frame_correction$type[i+1])
{

jj <- frame_correction[i:(i+1),]$value;
ll <- c();
for(h in 1: length(jj))
{
j <- nrow(xk[which(as.numeric(as.character(xk$LengthClass_cm))<=jj[h]),]);
ll <- rbind(ll, xk[j,])
}

jj_n_corrected <- ifelse(unique(frame_correction[i:(i+1),]$type)=="a", which(ll$freq==max(ll$freq)), which(ll$freq==min(ll$freq)));
jj_corrected <- jj[jj_n_corrected];

value_exclude <- c(value_exclude,jj_corrected);
}
}

if (!is.null(value_exclude)) frame_correction <- subset(frame_correction, !(value %in% value_exclude)) else
...
#frame_correction[2,1] <- 0;
#frame_correction[2,2] <- "a";
}

modes_robust <- subset(frame_correction, type=="m")$value;
antimodes_robust <- subset(frame_correction, type=="a")$value;
...
###
	...
	if (length(antimodes_robust)>0) amp_r <- abs(diff(xld_r)) else amp_r <- xld_r;

#print(subset(x, LengthClass_cm %in% ld_r))
#print(amp_r)
