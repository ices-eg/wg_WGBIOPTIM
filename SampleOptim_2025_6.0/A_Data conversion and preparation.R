...
input_dir <- file.path(this_dir, "data")

### Input settings file
sett <- read.table(file.path(input_dir,  "input_params.csv"), sep = ";",
                   header = FALSE, row.names = c(), stringsAsFactors = FALSE)
...
### convert parameters that can take FALSE/ TRUE as values to logical
stt <- stt%>%
  dplyr::mutate_at(c("SEX_RATIO","distUniPorto","PORT"),function(x) type.convert(x[1]))
