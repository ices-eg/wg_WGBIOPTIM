...
# Column prep [project specific]
	...
	# creates mature - definition of mature according to maturity stages
		...
		for (len in 1:length(IMMATSTAGES)) {
		  ...
		  df0$mature[!is.na(df0$matStage) & df0$matScale == IMMATSTAGES_this[1] & df0$matStage %in% IMMATSTAGES_this[2:length(IMMATSTAGES_this)]] <- 0
		  df0$mature[!is.na(df0$matStage) & df0$matScale == IMMATSTAGES_this[1] & !df0$matStage %in% IMMATSTAGES_this[2:length(IMMATSTAGES_this)]] <- 1
		}
		
		# df0$mature[!is.na(df0$matStage) & df0$matStage %in% IMMATSTAGES] <- 0
		# df0$mature[!is.na(df0$matStage) & !df0$matStage %in% IMMATSTAGES] <- 1
