...
myWD <- "C:\\STREAM\\WP3\\CASE STUDIES\\CaseStudy1\\BioSim Tool 1.01"
setwd(myWD)

caseStudy_path <-"C:\\STREAM\\WP3\\CASE STUDIES\\CaseStudy1"

species_name <- "Merluccius merluccius"  # "Aristeus antennatus"
short_name <-  "HKE" # "ARA"
...
# load input data
	load(paste(caseStudy_path, "\\input files\\input_data.rdata", sep=""))
	...
	if (	GSA != "ALL") {
	  res_dir <- paste(caseStudy_path, "\\C_maturity_", short_name, "_", GSA, "_",str_cat, "_",str_gear, "_",str_quart, sep="")
	} else {
	  res_dir <- paste(caseStudy_path, "\\C_maturity_", short_name, "_",str_cat, "_",str_gear, "_",str_quart, sep="")
	}
	
	if (	SEX != "ALL") {
	  res_dir <- paste(res_dir, "_", SEX, sep="")
	} 
...
if (by_EUlev6=="N" & by_EUlev4=="N") {
	...
	df0_by_year <- data.frame(summarise(df0_by_year,  maturity_measures=length( mature)))
	
	df0_avg <-  group_by(df0_by_year , Metier_Gear,quarter,commCat )  
	df0_avg <- data.frame(summarise(df0_avg,  maturity_meas=round(mean( maturity_measures), 0)))
	...
	write.table(past_situation,paste(res_dir,"/Past situation_", SEX,".csv", sep=""),sep=";",row.names=F)
...
df1=df0[df0$matStage!="",]	

tab=aggregate(df1$matStage,by=list(df1$sampId),FUN="length")	

colnames(tab) =c("sampId","Num_ind_with_matstage") 
summary(tab[,2])

write.table(tab, paste(res_dir,"/Numbers with matStage by sampId.csv", sep=""),sep=";",row.names=F)
...
write.table(QUANTS, paste(res_dir,"/Numbers with matStage by sampId_quantiles.csv", sep=""),sep=";",row.names=F)
...
# ====================
	...
	ls_sims_stats<-lapply(sapply(as.character(sampling_options$samp_sizes), function(x) NULL), function(x) sapply("mature_proportion", function(x) NULL)) 

	vars_numerical<- c("maturity") #variable_table$variable[variable_table$type=="numerical"]
	vars_categorical<-c() # variable_table$variable[variable_table$type=="categorical"]
	...
	BASELINE_VALUE_tab <- data.frame(summarise(BASELINE_VALUE_tab, BASELINE_VALUE= mean(Num_ind_with_matstage) ))
	...
	for (sampId in samples_to_analyze)  {
	  ...
	  if (sampId == samples_to_analyze[1]) {
	    total_mature_res <-	do.call("rbind", ls_DT_compiled[[sampId]])
   
	  } else {
	    total_mature_res <- rbind(total_mature_res, do.call("rbind", ls_DT_compiled[[sampId]]))
...
total_mature_res$mature <- as.numeric(as.character(total_mature_res$mature))


if (by_cat=="Y"){
  df0_ok=unique(df0[,c("sampId","commCat")])
  total_mature_res=merge(total_mature_res,df0_ok,by=c("sampId")) 
}
...
if (by_cat=="N"){
  total_mature_res$commCat="-1"
}

if (by_EUlev6=="N" & by_EUlev4=="N") {
  total_mature_res$Metier_Gear="-1"
} else if (by_EUlev6=="Y") {
  total_mature_res$Metier_Gear=total_mature_res$foCatEu6_sc
} else if (by_EUlev4=="Y") {
  total_mature_res$Metier_Gear=total_mature_res$foCatEu4
}

if (by_quart=="N"){
  total_mature_res$quarter="-1"
}

total_mature_res <- total_mature_res[, !(colnames(total_mature_res) %in% c("foCatEu6_sc", "foCatEu4")) ]

total_mature_res_prop_mat <- group_by(total_mature_res , Metier_Gear, quarter, samp_size,	n_it, lenCls, commCat) 



write.table(total_mature_res_prop_mat,paste(res_dir,"/Summary_table_by_length_maturity_", SEX,".csv",sep=""),sep=";",row.names=F)

total_mature_res_prop_mat_ <- data.frame(summarise(total_mature_res_prop_mat,  matures=sum(mature), 
                                                  total= length(mature) ))

total_mature_res_prop_mat_$immatures <- with(total_mature_res_prop_mat_, total-matures)

total_mature_res_prop_mat_$pi =   with(total_mature_res_prop_mat_, matures/total) # mat
  total_mature_res_prop_mat_$qi =  with(total_mature_res_prop_mat_, immatures/total) #imma
  
  total_mature_res_prop_mat_$prod_pi_qi =  with(total_mature_res_prop_mat_, pi*qi) #imma
  
  
  
  total_mature_res_prop_mat_2 <- group_by(total_mature_res_prop_mat_ , Metier_Gear, quarter, samp_size,	n_it, commCat) 
  
  

  
  total_mature_res_prop_mat_3 <- data.frame(summarise(total_mature_res_prop_mat_2,  CV=sqrt(sum(prod_pi_qi)/(sum(total)-1))/ (sum(matures)/sum(total))*100     ))
  write.table(total_mature_res_prop_mat_3,paste(res_dir,"/Extended_output_CV_maturity_", SEX,".csv",sep=""),sep=";", row.names=F)
  
  
  dataset<-total_mature_res_prop_mat_3
...
samp= df0[,c("sampId","foCatEu6_sc","foCatEu4","quarter","commCat","lenCls","mature")]
samp$mature=as.numeric(as.character(samp$mature))
...
samp2 <- data.frame(summarise(samp_,  matures=sum(mature), total= length(mature) ))

samp2$immatures <- with(samp2, total-matures)

samp2$pi =   with(samp2, matures/total) # mat
samp2$qi =  with(samp2, immatures/total) #imma

samp2$prod_pi_qi =  with(samp2, pi*qi) #imma

samp3 <- group_by(samp2 , Metier_Gear, quarter, commCat)  

samp4 <- data.frame(summarise(samp3,  CV=sqrt(sum(prod_pi_qi)/(sum(total)-1))/ (sum(matures)/sum(total))*100))
...
dataset2<-total_mature_res_prop_mat_
...
for (mett in met){
  ...
  for (quartt in quart){
    ...
    for (catt in categ){
      ...
      this <- dataset[dataset$commCat==catt & dataset$ Metier_Gear==mett & dataset$quarter==quartt, ] 
      ...
      b + geom_point(stat = "identity") +  xlab("") + ylab("")   + ylim(c(0,max(this$CV))) + 
        ...
        ggtitle(paste(species_name, "- cat=",catt, "met=", mett, "quarter=", quartt))  + 
      ...
      ggsave(paste(res_dir,"/Cat_",as.character(catt),"_met",mett,"_Q",quartt,"_CV_maturity_", SEX,".jpg", sep=""), 
             last_plot(),  height=8, width=8) 
    }
    }
...
for (mett in met){
  ...
  for (quartt in quart){
    ...
    for (catt in categ){
      ...
      b + geom_point(stat = "identity") +  xlab("") + ylab("") + facet_grid(.~samp_size)   + ylim(c(0,max(dataset2$pi))) + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
        ggtitle(paste(species_name, "- cat=",catt, "met=", mett, "quarter=", quartt))  
      ggsave(paste(res_dir,"/Cat ",catt,"_met",mett,"_Q",quartt,"_maturity_at_length_", SEX,".jpg", sep=""), 
             last_plot(),  height=8, width=16) 
      }
        ...
        for (quartt in quart){
            for (catt in categ){
              ...
              b + geom_point(stat = "identity") +  xlab("") + ylab("") + facet_grid(.~samp_size)   + ylim(c(0,max(dataset2$pi))) + 
                theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
                ggtitle(paste(species_name, "- cat=",catt, "met=", mett, "quarter=", quartt))  
              ggsave(paste(sim_dir,"/Cat ",catt,"_met",mett,"_Q",quartt,"_maturity_at_length_by_trip_", SEX,".jpg", sep=""), 
                     last_plot(),  height=8, width=16) 
              }
            }
...
summ_tab=aggregate(dataset$CV,by=list(Metier_Gear=dataset$Metier_Gear,quarter=dataset$quarter,
                                      samp_size=dataset$samp_size,commCat=dataset$commCat),FUN="mean")
...
for (i in c(1:nrow(tab))) {
  ...
  if (as.character(tab$samp_size[i])!="Baseline") {
      ...
      ref_prop_mat=overall_3[overall_3$samp_size == samp_sizes[1] & overall_3$Type == "Sampling" & 
      ...
      this_prop_mat=overall_3[overall_3$samp_size == as.numeric(tab$samp_size[i]) & overall_3$Type == "Simulations" &
                                     overall_3$commCat==tab[i,]$commCat & overall_3$Metier_Gear==tab[i,]$Metier_Gear & 
                                     overall_3$quarter==tab[i,]$quarter,c("lenCls", "mean_prop") ] 
      
      the_two_struct <- merge(ref_prop_mat, this_prop_mat, by =c("lenCls") , all=T)
      
      the_two_struct[is.na(the_two_struct[,])] <- 0
      
      tab[i,]$EMD <- round(emd2d(matrix(the_two_struct$mean_prop.x), matrix(the_two_struct$mean_prop.y) ), 4)
...
write.table(temp__,paste(res_dir,"/Summary_table_maturity_", SEX,".csv", sep=""),sep=";",row.names=F)

save.image(file=paste(res_dir, "\\C_maturity_output.RData", sep=""))
